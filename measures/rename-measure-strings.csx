/*
 * Title: Multi-string Search and Replace in Measure Names with Dependency Updates
 * 
...
foreach (var measure in measuresToProcess) 
{
    ...
    if (nameChanged) {
        ...
        measuresRenamed++;
        Info($"Renamed measure: '{originalName}' → '{newName}'");
    }
}

// Early exit if no measures were renamed
if (measuresRenamed == 0) {
    Info("No measures were renamed. Exiting.");
    return;
}
